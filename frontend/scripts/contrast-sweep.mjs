#!/usr/bin/env node
/**
 * contrast-sweep.mjs — Phase 1.5h multi-role palette WCAG sweep.
 *
 ...
 *   node frontend/scripts/contrast-sweep.mjs --out Blueprints/audits/2026-06-21-phase1-5h-multi-color-wcag-sweep.md
 */
...
function toMarkdown(rows) {
  ...
  const out = [];
  out.push('# Phase 1.5h — Multi-Color Palette WCAG Sweep');
  out.push('');
  out.push(`**Date:** 2026-06-21`);
  out.push(`**Phase:** 1.5h — Multi-role official palettes + special cultural variants`);
  ...
  for (const r of rows) {
    for (const [name, cell] of Object.entries(r.cells)) {
      if (!REQUIRED_CELLS.has(name)) continue;
  ...
  out.push('');
  out.push('| Team | Variant | Surface | Accent | Anchor | body/surf | accent/surf | CTA/acc |');
  out.push('|---|---|---|---|---|---|---|---|');
  for (const r of rows) {
    const c = r.cells;
    ...
    out.push(`| **${r.abbr}** ${r.name} | ${r.variant} (${r.variantName}) | \`${r.surface}\` | \`${r.accentHex}\` | ${r.anchor ?? '—'} | ${bs} | ${as} | ${ca} |`);
  }
  out.push('');
  ...
  out.push('     --out Blueprints/audits/2026-06-21-phase1-5h-multi-color-wcag-sweep.md');
  out.push('```');
  return out.join('\n');
}
