...
| [`build-football-intelligence-fixture.js`](build-football-intelligence-fixture.js) | Rebuilds the bounded Ben Johnson historical proof fixture from five SHA-256-pinned nflverse CSV assets. It validates source bytes, schemas, row counts, joins, unmatched rows, regular-season scope, deterministic order, and FTN attribution before writing the 4,217-row local fixture and manifest. With no arguments it downloads the public pinned assets to a temporary directory; `--source-dir <path>` uses an existing exact local snapshot. | Yes — public read-only downloads plus local fixture writes only; no package, credential, database, route, production host, deployment, or promotion behavior. It fails closed when a mutable upstream release asset no longer matches its recorded hash. |
...
node scripts/build-football-intelligence-fixture.js --source-dir /tmp/omen-football-intelligence-sources
```
...
| [`check-screen-reachability.mjs`](check-screen-reachability.mjs) | Every screen a `ScreenshotScenarios` entry mounts must also be referenced by production code. A scenario mounts a screen directly against fixtures, so it is **not** evidence of a route — a screen referenced only by the registry photographs perfectly and no user can reach it. Found by hand four times across 2026-09-18/19 (`OmenNoLeagueScreen`, `OmenConnectFailedScreen`, `OmenStartSitScreen`), including twice in journeys already closed as complete. Reads source only; writes nothing; no network. Exits 1 on a finding | Yes — local only |
