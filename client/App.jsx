...
/* ═══════════════════════════════════════════════════════════════
   Slops Saloon Fantasy Football MVP — App.jsx
   v3 Production — All bugs fixed:
   [1] CSS custom properties: en-dash (–) → double-hyphen (--)
   [2] Spread operators: ellipsis (…) → three dots (...)
   [3] Markdown code fences removed from JSX
   [4] Single export default at bottom
   [5] All imports at top
   [6] Dangling code outside components removed
   [+] PrivacyPolicy screen + Probo compliance footer integrated
═══════════════════════════════════════════════════════════════ */
...
.privacy-title { font-family: var(--font-d); font-size: 56px; font-weight: 700; letter-spacing: 2px; color: var(--bright); margin-bottom: 8px; }
.privacy-sub { font-size: 13px; color: var(--muted); margin-bottom: 48px; }
.privacy-section { margin-bottom: 36px; }
.privacy-h2 { font-family: var(--font-d); font-size: 22px; font-weight: 700; letter-spacing: 1px; color: var(--bright); margin-bottom: 12px; border-left: 3px solid var(--accent); padding-left: 12px; }
.privacy-p { font-size: 14px; color: var(--text); line-height: 1.75; margin-bottom: 10px; }
.privacy-table { width: 100%; border-collapse: collapse; margin: 16px 0; }
.privacy-table th { font-size: 10px; letter-spacing: 2px; text-transform: uppercase; color: var(--muted); text-align: left; padding: 8px 12px; border-bottom: 1px solid var(--border2); }
.privacy-table td { font-size: 13px; color: var(--text); padding: 10px 12px; border-bottom: 1px solid var(--border); vertical-align: top; }
.privacy-table tr:last-child td { border-bottom: none; }
.probo-badge {
  display: inline-flex; align-items: center; gap: 8px;
  background: rgba(0,214,143,.08); border: 1px solid rgba(0,214,143,.2);
  border-radius: 6px; padding: 10px 16px; font-size: 12px; color: var(--accent);
  margin-top: 8px;
}
...
.app-footer a { color: var(--muted); text-decoration: underline; cursor: pointer; background: none; border: none; font-size: 11px; transition: color .2s; }
...
async function fetchHistory(userId, platform) {
  // PRODUCTION: const r = await fetch(`/api/moves?userId=${userId}`);
  return simulateLatency(HISTORY_FALLBACK, 600);
}

async function fetchStandings(platform, leagueId, teamName) {
  // PRODUCTION: const r = await fetch(`/api/league/standings?platform=${platform}&leagueId=${leagueId}`);
...
/* ══════════════════════════════════════════════════════════════════
   SECURE API BRIDGE
   NOTE: In production, all AI calls go through your backend (/api/move)
...
// ── LANDING ──
function Landing({ onStart, onSub, onPrivacy }) {
  return (
    <div className="land">
...
function Onboarding({ onDone }) {
  ...
  const connectESPN = async () => {
    ...
    setConnError("");
    try {
      // PRODUCTION: POST /api/auth/espn/connect
      await new Promise(r => setTimeout(r, 1200));
  ...
  const handleFinish = () => {
    ...
    onDone({ platform:plat, leagueId:chosenLeague.id, leagueName:chosenLeague.name, teamName:sleeperUser || "My Team", scoring });
  };
    ...
    <div className="ob">
      ...
      {step === 2 && plat === "espn" && (
        <div className="ob-card">
          ...
          <div className="ob-p">ESPN has no public OAuth API. Paste your ESPN_S2 and SWID cookies from your browser. <a href="https://github.com/cwendt94/espn-api/discussions/150" target="_blank" rel="noreferrer" style={{color:"var(--accent)"}}>How to find them →</a></div>
          <div className="form-stack">
            <div className="form-field">
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const move = await callManagerAgent({
        weather: ctx.weather, travel: ctx.travel, gametime: ctx.gametime,
        roster:  ctx.roster,  perf:   ctx.perf,   matchup:  ctx.matchup,
        scoring, record: "10-4"
      });
      setVerdict(move);
              ...
              <div className="a-step-body">
                ...
                <div className={`a-step-status s-${st}`}>
                  {st === "done"   ? `✓ ${agentData[a.id] || a.done(scoring)}` :
                   st === "active" ? a.wait :
                   st === "error"  ? "✗ Failed — using fallback data" : "Queued"}
                </div>
              </div>
            </div>
          );
...
function ArchBanner() {
  ...
  const panels = {
    vault: {
      ...
      color: "var(--accent)",
      content: [
        "access_token, refresh_token, espn_s2 are encrypted at rest using pgsodium (AES-256-GCM).",
    ...
    hitl: {
      ...
      content: [
        ...
        "FeedbackCard requires 'Did you follow this move?' before stars can be submitted.",
        "followed=false moves are logged for UX insight but excluded from eff scoring.",
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8}}>
        {secCards.map(c => (
          <div key={c.id}
            ...
            style={{
              background: open === c.id ? "rgba(0,214,143,.06)" : "var(--bg)",
          ...
          <div style={{display:"flex", flexDirection:"column", gap:7}}>
            {panels[open].content.map((line, i) => (
              <div key={i} style={{display:"flex", gap:10, alignItems:"flex-start"}}>
                ...
                <span style={{fontSize:11, color:"var(--text)", lineHeight:1.55,
                  fontFamily: line.startsWith("CREATE") || line.startsWith("SELECT") || line.startsWith("WHERE") || line.startsWith("Key pattern") ? "monospace" : "inherit"
                }}>{line}</span>
              </div>
            ))}
          </div>
...
// ── FEEDBACK CARD — Human in the Loop ──
  ...
  return (
    <div className="state-card">
      ...
      {isAccept && (
        <div style={{
          background:"var(--bg)",
          border:`1px solid ${followed === null ? "var(--border2)" : followed ? "rgba(0,214,143,.3)" : "rgba(255,77,106,.3)"}`,
          borderRadius:10, padding:"20px 20px 16px", marginBottom:16, transition:"border-color .3s"
        }}>
    ...
    <div className="stand-panel">
      <div className="stand-head">
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between"}}>
          ...
          {!loading && !error && <span style={{fontSize:10, color:"var(--accent)", letterSpacing:1}}>{platform?.toUpperCase()} LIVE</span>}
        </div>
      </div>
      {loading && (
...
// ── DASHBOARD ──
function Dashboard({ user, onSub, onPrivacy }) {
  const [isSubscriber, setIsSubscriber] = useState(true);
  const [phase,  setPhase]  = useState("agents");
  const [move,   setMove]   = useState(null);
  const [fbDone, setFbDone] = useState(false);
  const [toast,  showToast] = useToast();

  const [history, histLoading, histError, addMove] =
    usePersistentHistory(user.leagueId, user.platform);

  const handleAgentsReady = (m) => { setMove(m); setPhase("move"); };
  ...
  const handleAccept = () => {
    ...
    addMove({
      ...
      scoring: user.scoring, source:"live", followed:null
    });
  ...
  const handleSkip = () => {
    ...
    addMove({
      ...
      scoring: user.scoring, source:"live", followed:false
    });
    ...
    <div className="dash">
      <div className="topbar">
        ...
        <div className="tb-right">
          ...
          <button className={`sub-pill${isSubscriber ? "" : " locked"}`}
            onClick={() => isSubscriber ? setIsSubscriber(false) : onSub()}>
    ...
    <div style={{position:"relative", zIndex:1}}>
      <div className="privacy-wrap">
        ...
        <div className="privacy-section">
          <div className="privacy-h2">What We Collect & Why</div>
          <table className="privacy-table">
            ...
            <tbody>
              {[
                ["Email address",         "Account identification",                            "Until deletion"],
                ["Fantasy league ID",     "Connecting to your platform (Sleeper/Yahoo/ESPN)",  "Until deletion"],
                ["Weekly AI moves",       "Providing your recommendation + feedback loop",     "Until deletion"],
                ["Outcome scores (eff)",  "Self-improving model calibration",                  "Until deletion"],
                ["OAuth tokens",          "Authorizing API calls on your behalf",              "Encrypted, refreshed weekly"],
                ["IP address (consent)",  "GDPR/CCPA consent audit trail",                    "3 years (legal requirement)"],
              ].map(([d,p,r],i) => (
                <tr key={i}>
                  <td style={{fontWeight:600, color:"var(--bright)"}}>{d}</td>
                  <td>{p}</td>
        ...
        <div className="privacy-section">
          <div className="privacy-h2">Your Rights</div>
          <p className="privacy-p">Under GDPR (Article 15, 17, 20) and CCPA, you have the right to:</p>
          {[
            ["Access",      "Request a complete export of all data we hold about you via GET /api/user/export"],
            ["Deletion",    "Permanently delete your account and all associated data via DELETE /api/user/delete"],
            ["Portability", "Receive your data in a machine-readable JSON format"],
            ["Withdraw",    "Withdraw optional consent (marketing, analytics) at any time in Settings"],
          ].map(([title, desc], i) => (
            <p key={i} className="privacy-p">
              <strong style={{color:"var(--bright)"}}>{title}:</strong> {desc}
            </p>
          ))}
        </div>

        <div className="privacy-section">
          <div className="privacy-h2">Our Commitments</div>
          {[
            "We never sell your data to third parties.",
            "We never use your data for advertising.",
            "We never share your fantasy roster data with other users.",
            "OAuth tokens are encrypted at rest using Supabase Vault (AES-256). Plaintext is never stored in any database column.",
            "Your data is never used for any purpose beyond providing your weekly AI recommendation.",
          ].map((c,i) => (
            <p key={i} className="privacy-p" style={{paddingLeft:16, borderLeft:"2px solid var(--accent)", marginBottom:10}}>
              ✓ {c}
            </p>
          ))}
        </div>
        ...
        <div className="privacy-section">
          <div className="privacy-h2">Contact</div>
          <p className="privacy-p">
            Privacy requests, questions, or data concerns: <strong style={{color:"var(--accent)"}}>privacy@slopssaloon.com</strong>
          </p>
          <p className="privacy-p" style={{color:"var(--muted)", fontSize:12}}>
            For deletion requests, you may also use the in-app Delete Account option in Settings (coming soon) or contact the above email.
          </p>
        </div>
      </div>
    </div>
...
/* ══════════════════════════════════════════════════════════════════
   ROOT APP — single export default
══════════════════════════════════════════════════════════════════ */
export default function SlopsSaloonFFMVP() {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div className="app">
        {screen === "landing"  && <Landing  onStart={() => setScreen("ob")} onSub={() => setShowSub(true)} onPrivacy={() => setScreen("privacy")} />}
        {screen === "ob"       && <Onboarding onDone={u => { setUser(u); setScreen("dash"); }} />}
        {screen === "dash" && user && <Dashboard user={user} onSub={() => setShowSub(true)} onPrivacy={() => setScreen("privacy")} />}
        {screen === "privacy"  && <PrivacyPolicy onBack={() => setScreen(user ? "dash" : "landing")} />}
        {showSub && <SubModal onClose={() => setShowSub(false)} onSubscribe={handleSubscribe} />}
        <Toast toast={toast} />
        <footer className="app-footer">
          © {new Date().getFullYear()} Slops Saloon Fantasy Football MVP ·{" "}
          <button className="app-footer" onClick={() => setScreen("privacy")} style={{display:"inline"}}>
            Privacy & Compliance
          </button>
          {" "}· 🛡️ Probo Verified
        </footer>
