...
Corvus uses only the documented read-only league draft, draft metadata, and draft picks endpoints. Sleeper documents no token requirement and a general ceiling below 1,000 calls per minute. Draft `last_picked` is an event timestamp; pick position comes from `pick_no`. A league can contain multiple drafts.
