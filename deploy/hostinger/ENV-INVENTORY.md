...
| `APP_BASE_URL` | Public app URL for redirects (OAuth callbacks, etc). | App | No |
...
| `VITE_APP_STORE_BUILD` | Build-time flag that produces an ESPN-connect-free bundle for mobile app-store builds; suppresses ESPN connect everywhere. Not yet wired into deploy.yml build args. | App | No |
