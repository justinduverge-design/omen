...
- **Yahoo server-mediated return:** completed in PR #191 / `dbafc66`. A server-verified native OAuth transaction returns only to `com.slopssaloon.omen://auth/callback?status=connected` or `?status=cancelled`; OAuth artifacts never enter the app link. Invalid, expired, and duplicate state fail closed without a deep-link redirect. Provider-console callback registration and real-device browser-to-app evidence remain separate human gates.
