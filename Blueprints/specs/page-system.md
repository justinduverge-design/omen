...
Headlines and brand display: **Cormorant Garamond** (serif). Everything else (UI labels, body, buttons, inputs, nav, meta, table cells, chip text): **Alegreya Sans** (sans). No other typefaces. No system serif fallbacks visible in production — both fonts are loaded in `frontend/src/index.css`; verify the `@font-face` declarations resolve before declaring a page font-fixed.

A page header in `<h1>` is always Cormorant Garamond 600 at 30–36px. A card headline (`<h2>` / `<h3>` inside a content card) is Cormorant Garamond 500 at 20–24px. The Omen recommendation title is Cormorant Garamond 600 at 24px. The TE1 chip "1" character must be at the same size as the position glyph (Justin QA: "should the '1' be at least the same size as 'TE'?" — yes).
