...
| `/trade` | `TradeAnalyzer.jsx` | Alegreya Sans page title + result card title. Body in Alegreya. | **Accent active** — Compare Trade CTA and share-card CTA in team accent; selected scoring/deal-shape chips use team accent; position buttons use position palette when unselected and team accent when selected; result accept/reject pills in team accent + risk hue | "Depth wins championships" copy revision remains Phase 2.13; share copy says public snapshot only and excludes connected-platform/private-league context | **Phase 2.10 shipped locally 2026-07-04** — result card can create a public share link after a successful comparison. **Phase 2.12 shipped locally 2026-07-05** — form now uses position buttons, explicit scoring format, honest Multi-team net-side mode, desktop swap cue, VORP help, and shared mock labeling. Still needs Phase 2.13 strategy-copy rewrite. |
...
| `/waiver` | `WaiverWire.jsx` | Alegreya Sans "Waiver Wire" section labels. Body/table rows in Alegreya. Pickup name in Alegreya Sans semibold. | **Accent active** — Get Picks CTA and week-input focus ring in team accent | "Waiver Wire" / "Top Pickups — Week N" — no dedicated brand line yet, tracked as a Phase 2.13-adjacent copy follow-up | **Phase 2.18 shipped (2026-07-05)** — route unlocked from behind the doctrine-stale Pro gate (Omen is free indefinitely), position chips migrated to the shared `positionChipStyle()` palette (Phase 1.6), and the page's `slate-*`/`amber-*`/`emerald-*` literals were swept onto design-system tokens |
...
| `/waiver` | inert — not registered in any moment's scope; recommendation-style list surface, same posture as `/trade` |
| `*` | inert |
