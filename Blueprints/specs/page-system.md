...
| `/onboarding` | `Onboarding.jsx` | Alegreya Sans success headline. Body in Alegreya. | **Accent active on the new "Pick your look" first step only** (Mode picker + 32-team grid + Continue/Skip CTAs consume `--color-team-accent`, same as `/account/appearance`) — Welcome / Connect / Complete steps remain accent inert pending the Phase 1.5 whole-app sweep | "You're set." / "League connected. Omen reads the matchup the moment your roster locks — your first call lands Tuesday." (Phase 1.10B, shipped 2026-06-25 via `slops-ux-copy`) | **Phase 1.5b shipped (2026-06-17)**; Phase 1.4 (font) shipped 2026-06-16; **Phase 1.10B copy shipped 2026-06-25** |
| `/account/connect` | `ConnectLeague.jsx` | Alegreya Sans "Connect Your League" headline. Body in Alegreya. | Accent inert (pre-team) | "Omen needs your league to find your Most Valuable Play." (current — approved) | **Phase 1.7 shipped 2026-07-01** — platform cards now use prominent Sleeper/Yahoo/ESPN brand color treatment via shared tokens. |
| `/account` | `Account.jsx` | Alegreya Sans page title "Account". Section headers Alegreya Sans. Body in Alegreya. | **Accent active** — header underline / focus rings should reflect team color | Alegreya Sans "Account" + section labels — TE1 sizing rule applies to any pill text | **Phase 1.7 shipped 2026-07-01** — Connect Sleeper / Yahoo / ESPN buttons and connected badges now share one shape/size system with platform-brand colors. Phase 2 backlog: subscription card removal for free era. |
...
| `/standings` | `Standings.jsx` | Alegreya Sans "Standings" headline. Table cells in Alegreya Sans. | **Accent active** — header rule, "you" row highlight in team accent | "Where every manager sits after the last whistle." (current — approved) | **Phase 1.7 shipped 2026-07-01** — platform badge now uses shared Sleeper/Yahoo/ESPN brand colors. Still **needs Phase 1.12** (W-L / PF / PA column legibility; "DarthSlops · you" row distinct treatment). |
...
|---|---|---|
| Sleeper | `#1FA3E8` | Connect tile, `/account` connect-row CTA + connected badge, Standings platform badge, league switcher chip |
| Yahoo | `#6E2E89` | Same set of sites |
| ESPN | `#CC0000` | Same set of sites |

Shipped 2026-07-01: the shared frontend helper `frontend/src/lib/platformBrand.js` now owns these exact values and applies them across `/account/connect`, `/account`, `/standings`, and the Sleeper league-switcher states. Connect-row CTA buttons share one shape + size across platforms; the color changes by platform only.
...
5. **`/onboarding` success page "You're ready" treatment** — copy + serif applied wrong, gray paragraph illegible. → **Phase 1.4 (font, done)** + **Phase 1.10B copy shipped 2026-06-25** ("You're set." / reads the matchup the moment your roster locks) + **Phase 1.12** (gray contrast, still open).
