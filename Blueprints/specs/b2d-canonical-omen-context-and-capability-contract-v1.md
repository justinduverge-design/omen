...
| Sleeper | Existing adapter builds the authenticated user's normalized roster with league settings and projections. | Live normalized free-agent/waiver pool is available for evidence-backed candidates. | `fetchSleeperLeagueRosters()` returns a sanitized all-team surface; a deterministic one-for-one evaluator requires both optimized lineups to improve and applies the existing VORP fairness guard. | Start/Sit, waiver, and trade code is merged for the selected Sleeper context; drafted-league evidence is aggregate-only. No separate production-route verification is claimed. |
| ESPN | Existing adapter builds the selected authenticated roster using server-side credentials and optional team ID. | PR #265 (`171508f`) normalizes the selected-context pool; PR #266 (`623068a`) evaluates it in canonical Omen. Drafted-league proof found 0 rostered-player leaks and 0 non-zero `onTeamId` entries in a 500-entry filtered pool. | Adapter exposes standings but no normalized opponent-roster trade candidate surface. | Start/Sit and fixture-backed waiver code are merged. The aggregate provider proof supports ownership filtering; no deployment or production-route verification is claimed. Trade remains unavailable. |

This matrix distinguishes merged code, fixture evidence, and sanitized provider proof from deployment or production-route verification. It is not a public all-platform-live claim.
