...
| Sleeper | Existing adapter builds the authenticated user's normalized roster with league settings and projections. | No normalized free-agent/waiver-pool capability exists in the adapter. | Adapter can read league rosters/users internally, but exports no opponent-roster trade surface or candidate evaluator. | Start/Sit only; waiver/trade unavailable until a dedicated live adapter contract is built and tested. |
