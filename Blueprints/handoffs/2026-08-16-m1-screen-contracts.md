...
**Merged** as PR [#321](https://github.com/justinduverge-design/omen/pull/321) / `38771f3`, 2026-08-17T01:06Z — **into its base branch `feat/m5-slice-e-ledger`, not into `main`**, because GitHub retargets a stacked PR only when its base branch is deleted and this one was not. The code track landed on `main` as #320 / `ee4387f` 25 minutes earlier, so for that window `main` carried the code track alone. Re-landed on `main` by cherry-pick as PR #322. **The stacking gave a real audit trail and a real footgun; if this pattern is used again, delete the base branch on merge or open the second PR against `main` once the first lands.** Not deployed.
