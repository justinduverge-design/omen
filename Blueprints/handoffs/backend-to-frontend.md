...
## Yahoo league binding — 2026-08-13

- **Status:** implemented, unit-tested against fixtures; not deployed. Addresses `P1-YahooLeagueBinding` — the root cause of Yahoo never reaching a usable state (`Direction/current_sprint.md`).
- **New routes:**
  - `GET /api/yahoo/leagues` (bearer auth) — returns `{leagues: [{league_id, name, season}]}`, the authenticated user's current-season Yahoo NFL leagues via a live Yahoo API call. `league_id` is Yahoo's `league_key` (e.g. `449.l.123`) — the same value every other Yahoo route/adapter already expects as `league_id`/`leagueKey`. No separate raw `league_key` field is exposed.
  - `POST /api/yahoo/league` (bearer auth) — body `{leagueId}`. Validates the submitted id against a fresh `GET /api/yahoo/leagues` call before persisting, so a caller can't bind an arbitrary league. `400` if not one of the user's leagues or `leagueId` missing; `404` if no Yahoo connection exists yet (mirrors `GET /api/yahoo/roster`'s existing error shape). Updates only `platform_connections.league_id` — does not touch tokens, and works whether the existing row already has the `"yahoo"` placeholder or a stale league.
- **`league_id: "yahoo"` placeholder is unchanged, on purpose.** `platform_connections.league_id` is `NOT NULL` in Supabase; `"yahoo"` is a deliberate sentinel `hasUsableLeagueId()` already rejects, not a bug to "fix" by writing `null`. See `Direction/decision_log.md`.
- **`GET /api/platforms` and `GET /api/platforms/state` now correctly treat that placeholder as no league selected**, not a phantom bound league or a fully `connected` state — both previously used a naive truthy check on `league_id` instead of `hasUsableLeagueId()`. `leagues: []` and `state: "choosing_league"` / `error_code: "yahoo_league_context_missing"` are existing contract values this now actually reaches for a placeholder-only Yahoo row.
...
| Yahoo | `priority`, verified | Probed live 2026-09-06 against two bound leagues. **FAAB branch unobserved** — no Yahoo FAAB league was available, so a Yahoo FAAB league still returns `not_determined` and fails closed |

**Build the UI against all three `system` values now.** All three providers return real values
today, with no further contract change — so a client that
