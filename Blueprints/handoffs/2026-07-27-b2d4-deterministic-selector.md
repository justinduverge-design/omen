...
**Recovery branch:** `codex/selector-recovery`, rebuilt from the original stacked selector after S1/S2 landed.
...
| GREEN | Focused selector/route/service 55/55; full `npm test` **469/469** after current-main reconciliation |
...
- Yahoo's availability-only fallback is preserved when no scoreable selector candidate exists; it remains null-delta and fixture-verified pending Yahoo API reapproval.
- ESPN untouched — blocked on the E0 spike.

## Merge order

The original stack's base branch was deleted after #215, automatically closing #216/#217. Recovery landed #238 (S2) and #239 (S0/S1 closeout) before this selector branch. The Yahoo fallback merge overlap was reconciled explicitly rather than discarded.
