...
- Font system locked — Barlow Condensed (`font-display`, h1/h2) + DM Sans (`font-sans`, body/UI) + DM Mono (`font-mono`, data/numbers) + Cormorant Garamond (`font-serif`, brand moments only). Replaces previous Cormorant Garamond + Alegreya Sans dual-serif system. Tailwind `fontFamily` updated, Google Fonts import updated, base `@layer` rule locks h1/h2 to `font-display`. Closed 2026-05-30.
...
- Trade Analyzer form rework approved — Position-first row layout + player name autocomplete using `searchPlayers(position, query)` from `nflPlayers.js`. Phase 1 is frontend-only, no backend dependency. Column order changes to `[72px_1fr_88px_96px_32px]` (Pos → Name → Proj → Status → ×). Remove button becomes icon-only `×`. Mobile: two-row stacked layout. Closed 2026-05-30.
