...
**One coverage observation, flagged rather than acted on because it is outside this package's scope.** `pr-quality.yml` is path-filtered to `src/**`, `test/**`, `evals/**`, the package manifests, `frontend/src/**`, and itself — **`scripts/**` is not in the list.** So the two load scripts added and changed here are ungated by PR CI, as is every other script in that directory. That is the same class of hole the workflow's own header says it was written to close (#206, #252 reached green through path-filter gaps). It matters less than it sounds — nothing in `src/` imports these scripts and they have no tests — but the next person to put logic in `scripts/` should know it is unwatched. Worth a small item; not taken here, because widening CI config was not in S3, S4, or O4.
