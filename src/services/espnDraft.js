...
const ESPN_DRAFT_LIST_VERSION = "espn-draft-list.v1";
const ESPN_DRAFT_META_VERSION = "espn-draft-meta.v1";
const ESPN_DRAFT_STATE_VERSION = "espn-draft-state.v1";
...
const ESPN_DRAFT_ID_PREFIX = "espn:";
...
function buildEspnDraftId(leagueId) {
  return `${ESPN_DRAFT_ID_PREFIX}${String(leagueId || "")}`;
}

function parseEspnDraftId(draftId) {
  const raw = String(draftId || "");
  if (!raw.startsWith(ESPN_DRAFT_ID_PREFIX)) return null;
  const leagueId = raw.slice(ESPN_DRAFT_ID_PREFIX.length).trim();
...
function buildDraftListResponse({ leagueId, draft, drafts }) {
  ...
  return {
    contract_version: ESPN_DRAFT_LIST_VERSION,
    generated_at: nowIso(),
...
function buildDraftMetaResponse({ draftId, draft }) {
  return {
    contract_version: ESPN_DRAFT_META_VERSION,
    generated_at: nowIso(),
...
function buildDraftStateResponse({
  ...
  return {
    contract_version: ESPN_DRAFT_STATE_VERSION,
    generated_at: nowIso(),
    draft_id: String(draftId),
    status,
    type: draft?.type || null,
