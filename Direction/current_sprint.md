...
- **Status:** IN_PROGRESS — **implementation and tests complete**; the live GlitchTip delivery proof is founder-gated (see "Remaining" below).
...
- **Remaining — founder-gated, two items:**
  1. **Which backend does the deployed `SENTRY_DSN` point at?** The only DSN on the founder's machine is `VITE_SENTRY_DSN`, pointing at **sentry.io** (`o4511928445960192.ingest.us.sentry.io`), not the GlitchTip instance O1b stood up. If production's `SENTRY_DSN` is also sentry.io, then O1b's GlitchTip deployment is not receiving anything from Omen and O8's payoff lands in the wrong tool. **This needs answering before O8 can be called done, and it may split off a task of its own.**
  2. **The live delivery proof.** Everything above proves capture, payload safety, and transmission. Confirming the event lands *in GlitchTip* needs a GlitchTip DSN — a secret, and therefore a founder action. GlitchTip itself is reachable (`http://100.98.81.0:8000/` → 200 over Tailscale).
