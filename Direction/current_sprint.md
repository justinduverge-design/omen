...
- **Note 2026-08-18 (reconciled onto `main` late — see `decision_log.md`):** the Sentry SaaS org created under `O1b`'s ratified split destination already has **iOS and Android projects provisioned with DSNs**, alongside the verified web one. Neither DSN is wired into a native app yet — no Sentry SDK exists in either codebase — so this is a head start, not a shortcut past the actual `Done when:` work (a deliberate crash on each platform, symbolicated, PII-free, within 60s).
- **Unblock 2026-08-18 — Android half PROVEN, not just unblocked.** Hand-rolled Sentry envelope integration (no SDK dependency, same choice O8 made for the backend), wired via `Thread.setDefaultUncaughtExceptionHandler` in `MainActivity.onCreate`. A real deliberate crash (`adb shell am crash`) reached the Android Sentry project in under 1 second — founder-confirmed directly in the Sentry dashboard: `android.app.RemoteServiceException$CrashedByAdbException` / "shell-induced crash", `ANDROID-1`, 2 events, **0 users** (confirming no PII attached). Stack trace is real class/method/line names (unminified debug build — inherently symbolicated, no ProGuard mapping question in play for this proof). **Overall `Status` stays `BLOCKED`** — `Done when:` requires both platforms and the iOS half is untouched, still gated on `TASK-R3-BUILD-iOS`. Evidence: `Direction/reviews/2026-08-18-o6-android-crash-reporting.md`; `Blueprints/handoffs/2026-08-18-o6-android-crash-reporting.md`.
