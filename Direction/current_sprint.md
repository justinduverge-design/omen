...
| M3-A | Native authentication proof | P0 | M3 ✅ + founder auth-config authority | **Required, not optional, before any external/native release.** Replace M3's local placeholder with iOS Sign in with Apple + email OTP and Android Credential Manager + email OTP, backed by Supabase secure storage and named failure/recovery states. Requires explicit configuration authority, safe real-device evidence, and no provider connection work. |
