...
| M0b | Mobile design-system contract | P0 | ~~M0a~~ **unblocked** | Tokens, component registry, accessibility, theme packs, and platform-specific implementation rules are approved. Source: Markdown specs (Design House Figma is a stub — `00 — Start Here` only — so Figma is a pending cross-reference). |
