...
- **Status:** **VERIFIED 2026-08-19 — both halves, founder-confirmed in the dashboard.**
...
- **Fix APPROVED and PREPARED 2026-08-19 — one command from landing.** The founder approved the deploy-workflow change (founder-gated per `known_issues.md` § Do Not Touch). It is written and verified but **not yet on `main`**: pushing `.github/workflows/**` needs the `workflow` OAuth scope, which this session's token lacks, and GitHub rejected the push. The change is preserved as an applicable patch at `Direction/reviews/evidence/2026-08-19-o2/deploy-immutable-tags.patch` — three edits: `:sha-${{ github.sha }}` added alongside `:main` for both the API and cron images, and `docker image prune -f` scoped to `--filter "until=168h"`. **Apply with `gh auth refresh -s workflow` then `git apply`, or apply the patch and commit from any terminal with the scope.**
- **The runbook already documents the post-fix path**, with the digest procedure kept as a documented fallback for builds pushed before the tag change — those carry no `sha-` tag — and the before/after account retained so a later "cleanup" trimming the tag list back to one entry can see what that costs. **Until the patch lands, the fallback path is the live one.**
