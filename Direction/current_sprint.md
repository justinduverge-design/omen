...
- **Status:** **IN_PROGRESS — slices A + B + C complete and verified on iOS 2026-08-15; Android parity for those three slices is the open half.**
...
- **Open:** **Android A + B + C parity** — `OkHttp`-based client, the same `dashboard-summary.v1` / `league-standings.v1` decode and honest-state mapping, view model, and `:app:assembleDebug` + connected-test evidence. Slices D–G unchanged and unstarted. Do not mark this item VERIFIED on the iOS half alone.
