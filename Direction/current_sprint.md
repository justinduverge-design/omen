...
| M1-F | Native Figma token and foundation-library setup | P0 | M1 focus-ring + typography briefs ✅ | **Unblocked.** Populate the official Design House only on `02 — Tokens & Themes` and `03 — Components`, using the approved registry and M1 briefs. Record semantic focus-ring, locked typography roles, and the registry's existing foundation-component anatomy/state/accessibility/platform mappings; propose no new token, component, or screen pattern. |
