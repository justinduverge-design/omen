...
- **Status:** READY
- **Blocked by:** TASK-A7-OwnedFootballDataPipeline — founder expanded the required architecture on 2026-08-24: it must support full league scoring contracts and exact provider-result reconciliation, not only base Standard/Half PPR/PPR derivation. The former A7 source/replay pass remains evidence, but is insufficient for a league-exact production claim.
- **Blocked by:** TASK-A6-MovesScoringFormat — founder re-scoped A6 on 2026-08-24 from reception-format capture to a full league scoring contract. Tuesday grading may not claim league accuracy until every material league rule is extracted, normalized, calculated, and reconciled under a named ruleset version.
...
- **Status:** IN_PROGRESS — re-scoped by founder 2026-08-24
- **Claim:** Codex, 2026-08-24 — founder directed that Omen extract and calculate full league scoring rules, not only Standard/Half PPR/PPR reception formats. The existing partial implementation is preserved as evidence and must not merge or deploy as the final solution.
- **Evidence:** 2026-08-24 — pre-change `npm test` was 618/618; the partial correction reached 626/626 and proves provider reception-format capture, move persistence, three base grades, and historical PPR compatibility. It does **not** capture or calculate all material rules such as configurable passing values, bonuses, fumbles, kicker, DST, or provider-specific adjustments. `Direction/reviews/2026-08-24-full-league-scoring-contract-adr.md` now defines the required immutable provider-rule snapshot, canonical contract, lawful event facts, provider-final reconciliation, coverage states, and acceptance gates. Review-only nullable SQL remains at `sql/2026-08-24_a6_moves_scoring_format_review.sql`; it was not applied.
- **Blocked by:** AGENT_RESOLVABLE — author and ratify the Full League Scoring Contract: provider settings snapshots; canonical rules DSL and rule-coverage matrix; stat-event model; provider-to-canonical mappings; versioning; exact-result reconciliation; custom/manual-adjustment behavior; privacy/retention treatment; and acceptance proof across Sleeper, ESPN, and Yahoo.
...
- **What is wrong:** the original defect is real—every move defaulted to PPR—but its initial scope was too narrow. A reception-format label cannot reproduce a league’s full scoring contract. Configurable passing, rushing, receiving, fumble, bonus, kicking, defense/special-teams, positional, and provider-specific rules can change a grade even when both leagues are called “PPR.”
- **Why it belongs to the vendor-agnostic ask:** league rules are a first-class, per-league contract. Public NFL facts supply events; a user-authorized provider supplies that league’s rules; Omen must apply the versioned contract and reconcile to the provider result before claiming exactness.
- **Done when:** for each supported provider, Omen snapshots the complete league scoring configuration at recommendation time; normalizes it into a versioned canonical rules contract; calculates every supported rule from lawful event facts; marks coverage exactly; reconciles computed outcomes to the authorized provider’s final result; visibly versions corrections; and refuses a league-exact claim when any material rule or manual adjustment cannot be reproduced. A provider without a supported lawful exact-score path emits its required user-facing provider-restricted explanation; ESPN uses Policy v1.4’s “Exact ESPN scoring unavailable” state. The original PPR fallback remains only for identifiable historical rows.
- **Do not touch:** merging or deploying the current three-format partial correction as a final league-accurate solution; applying SQL to staging or production; scraping, using unclear rights, or treating a provider’s user-authorized settings as a public stats source. The SQL gate remains approval → staging → verification → production.

### A7-OwnedFootballDataPipeline — Design the automated Slops-owned football-data pipeline

- **Status:** IN_PROGRESS — expanded by founder 2026-08-24
- **Claim:** Codex, 2026-08-24 — preserve the completed rights-first source/replay architecture as a base, then extend it into the founder-required Full League Scoring Contract before any collector/build authorization.
- **Evidence:** `Direction/reviews/2026-08-24-a7-owned-football-data-pipeline.md` remains valid for source rights, immutable inputs, identity, operations, and base-format formula replays. `Direction/reviews/2026-08-24-full-league-scoring-contract-adr.md` begins the expanded contract: it distinguishes private provider rules/outcomes from lawful public event facts; defines canonical rule/reconciliation states; and records current provider gates (Sleeper written commercial permission, Yahoo terms/attribution confirmation, ESPN express written permission). No collector, timer, deploy, dependency, credential, SQL, migration, or production change occurred.
- **Blocked by:** AGENT_RESOLVABLE — Full League Scoring Contract architecture and provider/rights research: complete scoring-config extraction for Sleeper/ESPN/Yahoo; canonical rules DSL; event/stat coverage; base/bonus/threshold/defense/kicker rules; commissioner/manual adjustment semantics; provider-result reconciliation; correction/notification behavior; support matrix; and exact acceptance evidence.
...
- **Scope:** evaluate at least five primary or openly licensed football-stat sources for licence, ToS, coverage, correction latency, identifiers, rate limits, and automation rights; design immutable raw snapshots → normalized player/game identities → complete provider-specific league scoring contracts → versioned derived results → provider-result reconciliation → Tuesday publication. Include scoring-config extraction for Sleeper/ESPN/Yahoo, canonical rule vocabulary/DSL, bonuses/thresholds, kicker/DST, custom/manual adjustments, coverage and correction semantics. Compare VPS-primary/Pi-witness, Pi-primary/VPS-failover, and VPS-only operating shapes; cost build and in-season maintenance; define monitoring, replay, correction, provenance, and source-loss behavior. Identify the clean extension seam for a future Slops-owned ADP corpus without treating ADP as part of this scoring deliverable.
- **Done when:** a source-backed architecture memo names the lawful source set, exact schedules, storage and retention, idempotency/replay rules, data-quality checks, infrastructure roles, failure and failover behavior, build estimate, weekly maintenance estimate, and a phased implementation plan; it also specifies full scoring-contract extraction, canonical rules, provider mappings, support/exception states, exact reconciliation, and correction handling. At least two historical weeks and each supported provider/rule family must be replayed in non-production proof before any production collector is proposed.
