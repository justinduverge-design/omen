...
- [ ] **Phase 2.17 follow-up — collapse the Sleeper/Yahoo `lastResult` lookup to fewer round trips.** `attachLastResult` (`src/routes/dashboard.js`) makes 3 sequential Sleeper calls (user lookup, roster lookup, matchups) and 2 sequential Yahoo calls (team-key lookup, matchups) per uncached dashboard request. Both are 6h-Redis-cached so steady-state cost is low, but the cache-miss path is the worst-case latency hit on a route already flagged for p95 watch. Surfaced by `slops-code-review` as P2 during Phase 2.17 (not blocking — see `Blueprints/handoffs/2026-06-29-phase2-17-platform-last-result-handoff.md`). Likely fix: Sleeper's `rosters` payload already carries `roster_id` per `owner_id`, so the separate `users` fetch inside `fetchSleeperRoster` may be avoidable for this specific lookup; Yahoo's `getMyTeamKey` result could itself be cached per user/league (it doesn't change mid-season). Done docs: feature + recommendation.
