...
| M3A-iOS | Native iOS auth implementation | P0 | M3-A Android ✅ + macOS CI ✅ (`ios-ci.yml`) | 🟡 **Implemented 2026-07-21 — GitHub issue #159, PR pending CI.** Keychain session, Sign in with Apple + email OTP, GoTrue via URLSession, account deletion, 37 XCTest cases (exact Android parity count); `ios-ci.yml` flipped build→test with pinned Xcode 16.2. No local macOS toolchain to compile with — **not yet CI-verified**; treat as unverified until `ios-ci.yml` is green on the PR. Evidence: `Blueprints/handoffs/2026-07-21-m3a-ios-auth-implementation.md`. Real-device Sign in with Apple + OTP-inbox QA remains founder/human (see M3A-QA). |
