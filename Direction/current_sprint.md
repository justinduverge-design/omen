...
| M2-E | Native build-environment decision | P0 | M2-F ✅ | **Unblocked.** Choose a non-purchase iOS build/validation path and an Android toolchain plan; current Windows workspace has neither Xcode nor Java/Gradle/Android SDK/ADB, and Justin will not buy a Mac. Done when the selected path names owner, cost posture, device/CI evidence, and what remains unavailable; no project scaffold or hosted-service spend without approval. |
...
| M2 | Native app-shell project scaffolding | P0 | M0 ✅ + M2-F ✅ + M2-E | iOS/Android project structure, auth/session, demo, navigation, environment, and store-safe configuration contracts are approved; scaffold only after the app-shell Figma contracts are approved and a build-environment plan is available. |
