...
| M1-P | Native primitives + component enforcement | P0 | M1 focus-ring + typography briefs ✅; authority reconciliation first | **Figma screen-contract pass APPROVED 2026-07-20 (Justin).** P2 foundations merged through PRs #165–#168: tokens, controls, fields, Card, Badge/Chip, Modal Sheet, and six State Surfaces. Each merged slice has Android build/device evidence and unsigned iOS simulator CI. **Remaining P2:** ListRow, PlatformBadge, and ConfirmationDialog; then P4 gallery/enforcement before P3 product compositions. Evidence: dated `Blueprints/handoffs/2026-07-20-m1p-p2-*.md`. No feature-local primitive clones or unapproved compositions. |
...
| M3A-iOS | Native iOS auth implementation | P0 | M3-A Android ✅ + macOS CI ✅ (`ios-ci.yml`) | 🟡 **Implemented + CI green 2026-07-21 — GitHub issue #159, PR #171 (draft, awaiting founder review/merge).** Keychain session, Sign in with Apple + email OTP, GoTrue via URLSession, account deletion, 37 XCTest cases (exact Android parity count); `ios-ci.yml` flipped build→test with pinned Xcode 16.2 and is now **green** (`xcodebuild test` passing) after 3 small fix-forward commits for pbxproj issues CI itself caught (wrong `TEST_HOST`/module name, missing `ENABLE_TESTABILITY`). Evidence: `Blueprints/handoffs/2026-07-21-m3a-ios-auth-implementation.md`. Real-device Sign in with Apple + OTP-inbox QA remains founder/human (see M3A-QA). |
...
- **M1-P P2 progress (reconciled 2026-07-21):** PR #165 merged tokens, controls, and fields;
  PR #166 merged Card/Badge/Chip; PR #167 merged Modal; PR #168 merged State Surfaces. The next
  small approved-registry slice is ListRow, followed by PlatformBadge and ConfirmationDialog.
