...
- **Evidence:** 2026-08-26 safety repair on `codex/a7b-closure-reconciliation` makes the server persist every issued live recommendation, marks feedback-only rows `scoring_contract_required=true`, refuses to issue a recommendation when persistence fails, and leaves uncaptured live scoring format `null` instead of inventing PPR. Focused 56/56 and full backend 713/713 passed. This is fail-closed containment, not full A6 completion, and is not production behavior until merged/deployed.
