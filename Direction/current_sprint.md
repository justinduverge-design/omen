...
- **Status:** READY — Android half `VERIFIED`; iOS half **implemented and unit-proven 2026-08-19, awaiting the DSN for its live proof.**
- **Blocked by:** the iOS Sentry DSN is not present on this machine. `mobile/ios/OmenIOS/Config/Local.xcconfig` (git-ignored) has no `OMEN_IOS_SENTRY_DSN`, and the value lives only in the founder's Sentry account. **Everything else on the iOS half is done.**
- **Unblock:** paste the DSN from Sentry → org `valor-ventures-llc` → project **iOS** → Settings → Client Keys (DSN) into `Local.xcconfig` as `OMEN_IOS_SENTRY_DSN = …`. The key already exists — `O1b` provisioned the iOS project on 2026-08-17. Then the deliberate-crash proof can run.
