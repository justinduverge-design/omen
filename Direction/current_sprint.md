...
1. **30 `CLOSED` tombstone stubs** were still occupying the active queue, each already carrying
   ...
   `sprints_completed.md` before removal; 28 have a real record there. Removed this pass.
2. **`R4` and `R5` were closed 2026-08-23 with no ledger row** — the exact defect the 2026-09-02
   pass flagged for `O2` and `W1-GATE`, recurring twice more and undetected. They appear in
   `sprints_completed.md` only as cross-references from other items' prose (*"R4/R5 still gate
   rollout"*), never as closures. **Founder call, not an agent's:** either the evidence exists and
