...
Last updated: 2026-06-15 (Phase 1.4 font system propagation closed locally; Phase 1.5 is the recommended next agent-buildable pull. Every unchecked agent-buildable item names its applicable Done docs.)
...
- [x] **Phase 1.4 — Font system propagation fix (2026-06-15).** Replaced the old Cormorant direction with Alegreya Sans for headings/card titles/UI and Alegreya for body text per Justin's brand decision. Updated the brand/design/page specs and applied the font propagation sweep across flagged pages/components. Fixed TE1 "1"-vs-"TE" parity by moving strategy titles to the heading/UI font. Fixed light-mode olive-gold "Continue with Email" token misuse. Verified `npm --prefix frontend run build` clean; `npm test` 291/291; targeted browser QA confirmed fonts load, no visible route computes Cormorant/Garamond, `/login` light email CTA uses `#92740F`, and 13 routed paths were checked. Guardrail: `slops-ui-ux-audit` no P0 by targeted screenshot/computed-style review. Done docs: page + design.
