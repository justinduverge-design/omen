...
- **⚠️ PROPOSED ACCEPTANCE AMENDMENT — 2026-08-27, awaiting founder ratification.** The `Done when:` below requires *every* recommendation to name an immutable provider-rule snapshot. **That is unsatisfiable for a legally restricted provider**, and an item whose acceptance cannot be met is not a high bar — it is an item that can never close, which pressures whoever is holding it toward fabricating a snapshot to satisfy the sentence. Proposed replacement for the first clause:

  > every recommendation names either (a) an immutable provider-rule snapshot and versioned canonical Scoring Contract, or (b) an immutable, hashed **restriction attestation** naming the provider, the coverage state, and the reason no lawful snapshot exists — never a fabricated or partial snapshot presented as complete;

  **The code already behaves this way**, which is why this is a wording fix rather than a build. `deriveScoringSnapshot({platform:"espn"})` returns `coverage_state: "provider_restricted"`, an explicit reason, `rules: []`, and a hash — an attestation, not a snapshot — and `reconcileMoveScoring` refuses `exact` for it regardless of the numbers. The acceptance sentence is the only part out of step. Raised by the parallel session 2026-08-27; **not applied to the clause below until the founder ratifies it.**
