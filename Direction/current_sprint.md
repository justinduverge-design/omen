...
Last updated: 2026-05-29 (session 4 — hamburger nav, team theme, HITL card, logo slot)
...
- Keep prepared Supabase SQL distinct from applied database state until Justin approves migration.
- Logo placeholder (`[C]` circle in header/drawer) is intentional — swap with SVG inline component when logo is ready.

## Next

1. Get Justin approval to apply `moves` table Supabase migration (unblocks HITL backend + Move History).
2. Get Justin approval to apply `profiles.favorite_team` column migration (unblocks Team Theme backend).
3. Run `/ui-ux-pro-max-skill` on Account page + ConnectLeague page — next UX gate.
4. Build Trade Analyzer form rework: position-first, autocomplete via `nflPlayers.js`, Trade Room column. No backend dep — can start immediately after Account/Connect audit.
5. Build HITL backend (`POST /api/omen/feedback`) — gated on moves migration.
