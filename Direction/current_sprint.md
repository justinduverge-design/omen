...
- **Status:** **VERIFIED 2026-08-19 (both halves) — pending founder dashboard confirmation for the iOS event**, matching the standard the Android half was held to.
...
- **Remaining to close:** the founder confirming the event in the Sentry **iOS** project dashboard, which is exactly the evidence the Android half carries (`ANDROID-1`, 2 events, 0 users). HTTP 200 proves Sentry *accepted* the envelope; only the dashboard proves it was *stored and rendered*. Do not mark `CLOSED` on the 200 alone.
