...
- **Phase 1.5 PR1 (team-theming system core) is in flight in a separate chat 2026-06-16.** Pinned in `Direction/agent_inbox.md`. Do not start parallel frontend work that touches `--color-accent`, `TeamTheme.jsx` / `Appearance.jsx`, the 8 "Accent active" pages, or `frontend/src/data/nflTeams.js` until PR1 lands. After PR1 merges, run `planning-pass` to formalize the 1.5b/1.5c/1.5d/Backend 2.17 split that was sketched in this sprint pass.
...
- [ ] **Phase 1.5 PR1 — Team-theming system core (IN FLIGHT 2026-06-16 in a separate chat).** Token model (`--color-team-primary/secondary/accent/surface/surface-card` in `frontend/src/index.css`) + runtime template resolver (`frontend/src/lib/teamTemplate.js` — 6 templates: Deep & Brand / Two-Tone Royal / Hot Brand / Aqua & Cool / Earth / Bred) + `TeamTheme.jsx` → `Appearance.jsx` rename with 3-button Mode picker (System / Team / Corvus, exclusive; Team always dark; System default for new users) + page sweep on the 8 "Accent active" pages (Account, Football, OmenPage, Ledger, Standings, TradeAnalyzer, DraftAssistant, Appearance) + voice extension to 6 surfaces (TeamTheme picker, NavDrawer label, Omen loading, Omen page subhead cry, Standings page subhead wardRoom, Dashboard header pill cultureTag) sourced from `frontend/src/data/nflTeams.js`. Saints get a primary↔secondary flip with extra gold weight. Falcons get Template 6 "Bred" (pure black canvas, varsity-red CTA, white hairline — Jordan 1 homage). 32-team contrast audit doc at `Blueprints/audits/2026-06-16-phase1-5-team-template-assignment.md`. Theme persists across signed-in sessions via localStorage. **Blocked by Phase 1.3 [x] + Phase 1.4 [x].** Guardrails: `ui-ux-pro-max` accent-contrast library; verdict: `slops-ui-ux-audit`. Done docs: page + design.
- [ ] **Phase 1.5b — Onboarding "Pick your look" first step.** New first step after sign-in: 3-button Mode picker (System / Team / Corvus, reuses the `Appearance.jsx` component) + 32-team grid + Skip button (lands the user on Corvus default; they can change anytime at `/account/appearance`). Theme persists via the localStorage keys written by Phase 1.5 PR1 so returning users land on the right look without re-picking. **Blocked by Phase 1.5 PR1 [ ].** Guardrails: `slops-ux-copy` for Skip + first-step microcopy; verdict: `slops-ui-ux-audit`. Done docs: page + design.
- [ ] **Phase 1.5c — Team voice extension into Ledger / Draft / Trade.** Apply the placement-guide pattern from `frontend/src/data/nflTeams.js` (lines 23-27) to three more surfaces: Ledger empty state → `wardRoom`, Draft Assistant intro line → `cry`, Trade Analyzer result accent → `cultureTag`. Pure frontend; no new copy authoring (phrases already exist in `nflTeams.js`). Renders only when mode = Team. **Blocked by Phase 1.5 PR1 [ ].** Guardrails: `slops-ux-copy` for placement review; verdict: `slops-ui-ux-audit`. Done docs: page + design + recommendation.
