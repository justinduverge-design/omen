...
- **Status:** READY
- **Blocked by:** FOUNDER_APPROVAL — destination ratification. The comparison memo is complete and routed; four options are costed (GlitchTip on Command Center backend-only / GlitchTip on KVM2 / Sentry SaaS free / **split destination, recommended**). The Sentry dependency of Options C and D is now **resolved — see below**; what remains is the founder's choice of destination and, under Option D, the GlitchTip backend half.
- **✅ 2026-08-17 — Sentry account exists and the web half is proven end to end.** The prior account was never lost; it never existed (`decision_log.md:634` recorded the intent in June, but `SENTRY_DSN` shipped blank and no account was created). A fresh org was created: **`valor-ventures-llc`**, ingest host `o4511928445960192.ingest.us.sentry.io`, free Developer plan with a 14-day unlimited-volume trial that reverts to free limits ~2026-08-31 (5,000 errors/month, 30-day retention, 1 seat, no card on file). Three projects exist: **React (web), iOS, Android**. Platform choices matter and are recorded deliberately: **Android** platform, *not* Kotlin language (the Android SDK is what carries ANR detection and ProGuard/R8 mapping upload — the `O6` symbolication clause depends on it); and **React**, *not* the auto-preselected React Router Framework (the frontend is `react-router` 8.3.0 as a library inside a Vite SPA, not framework mode). **No Express project was created** — under Option D the backend goes to GlitchTip, and keeping bursty backend volume off the metered path is the point of the split.
  - **Verification (web only):** `VITE_SENTRY_DSN` set in root `.env.local`; a deliberate uncaught error was captured and transmitted, `flush()` returned `true`, event `ab02073085b44260a949a8028fc22894`. **No code changes were required** — `main.jsx:8` already calls `initSentry()` and `sentry.js:144` is `enabled: Boolean(dsn)`. **The `@sentry/wizard` install step must not be run**: it would duplicate or overwrite the hand-written init and silently destroy the ESPN-credential scrubbers.
  - **`envDir` gotcha, recorded so the next session does not lose time to it:** `frontend/vite.config.js:10` sets `envDir: path.resolve(__dirname, '..')`, so Vite reads env files from the **repository root**, not `frontend/`. `VITE_SENTRY_DSN` belongs in `/.env.local`.
  - **iOS and Android are NOT verified and cannot be.** Neither native app contains a Sentry SDK. That is `O6` implementation work, not an onboarding step; the two DSNs are unused until then.
  - **⚠ Scrubber gap found by this verification — see `Direction/known_issues.md`.** The live `beforeSend` correctly dropped all headers and `[scrubbed]` every sensitive body key, but **did not scrub OAuth `code` / `state` from `request.url`** on the frontend. The backend does (`src/middleware/sentry.js:8`); the frontend has no equivalent pattern. This contradicts `decision_log.md:994`. Not fixed in this pass — security code, needs its own test. **This is an open item against this task's own "no PII, provider credential, or ESPN cookie in any captured payload" clause and must be closed before `O1b` can be `VERIFIED`.**
...
- **⚠ Do not default to self-hosted Sentry.** It wants ~16 GB RAM and a large compose stack (Kafka, ClickHouse, Snuba, Relay, Postgres, Redis). It **must not** run on KVM1 (3.8 GB, production, 49% disk), and would dominate KVM2 (7.8 GB, the AI host). It also does not fit Command Center — but see the correction below for why *GlitchTip* does.
- **✅ CORRECTED 2026-08-17 — two resource claims in this item were wrong, both in the same direction.** They overstated the cost of self-hosting and were the stated reason Command Center was excluded. Evidence: `Direction/reviews/2026-08-17-o1b-error-tracking-comparison.md`.
  - This item said GlitchTip wants **"roughly 1–2 GB."** Official install docs state **512 MB recommended**, **256 MB minimum** all-in-one, and "128 MB + swap" with careful configuration. arm64 is explicitly supported; PostgreSQL 14+ required; Valkey/Redis 7+ **optional**; ~30 GB disk per million events/month.
  - This item said Command Center has a **"<1 GiB budget."** That figure was never measured. The Slops OS Pi deployment tracker's own post-Kuma snapshot records **3.7 GiB total, 475 MiB used, 3.2 GiB available**, plus 107 GB free disk, Docker, aarch64. GlitchTip's recommended footprint is ~16% of measured free memory.
  - **Consequence:** Command Center is a viable host and this item's exclusion of it does not follow from the real numbers. Steward and Sentinel (Pi Zero 2 W, ~257/~265 MiB free) remain correctly excluded.
- **The SDK layer is already built — this is not an instrumentation task.** Phase 1.2 (2026-06-13) shipped `src/middleware/sentry.js` and `frontend/src/lib/sentry.js` with `@sentry/node` + `@sentry/react` at `^10.69.0`, full ESPN-credential/header/body scrubbing, and `test/sentryBoot.test.js`. `enabled: Boolean(process.env.SENTRY_DSN)`; `SENTRY_DSN` and `VITE_SENTRY_DSN` ship blank. **What is missing is a DSN pointing at a live backend.** GlitchTip is Sentry-SDK-compatible, so the existing instrumentation and every scrubber work unchanged against either destination — no client code changes in any option.
- **⚠ The real constraint is reachability, not RAM.** A Tailscale-bound host can satisfy this item completely and can satisfy `O6` **not at all** — a crash on a tester's phone on cell service has no tailnet route. Any option that lands everything on Command Center must say plainly that `O6` stays blocked rather than implying its blocker cleared. Public exposure of Command Center is rejected: it needs a residential port-forward, contradicts the Pi tracker's standing guardrails, and adds home public surface while `S6` is open to reduce public surface elsewhere.
- **Open question — Android symbolication on GlitchTip.** Its CLI documents `debug-files upload` for **dSYM, PDB, ELF** (iOS covered); **ProGuard/R8 mapping support is unconfirmed** and the CLI is explicitly pre-1.0. This is a direct risk to `O6`'s "symbolicated stack traces" clause for any option routing Android crashes to GlitchTip. Resolve before committing to one.
- **Carry in on ratification:** `decision_log.md:634` already recorded "SaaS Sentry free tier for error monitoring" as chosen tooling. This item was written later and reopens it as though nothing had been decided. Whichever destination wins needs an explicit supersede entry rather than a silent overwrite.
- **Evidence (research, not approval):** `Direction/reviews/2026-08-17-o1b-error-tracking-comparison.md`.
