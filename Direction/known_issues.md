...
- **✅ RESOLVED as to acquisition, 2026-09-19 — the entry above is stale in two ways and both are corrected here rather than in place, because the reasoning above is still the provenance.** (1) **The font files exist.** `mobile/ios/OmenIOS/OmenIOS/Fonts/WixMadeforDisplay.ttf` / `WixMadeforText.ttf` and `mobile/android/core/designsystem/src/main/res/font/wix_madefor_{display,text}.ttf` are committed. The claim "there are no font files in this repo" is false as of today. (2) **The locked families are no longer Alegreya/DM Mono.** Fact-of-record #16, amended 2026-09-13, locks **Wix Madefor Display + Text**. The entry above predates that amendment.

- **⚠️ The Dynamic Type finding's stated trigger has fired and the finding is still open — and the obvious fix is a trap that has already shipped once ([#338](https://github.com/justinduverge-design/omen/issues/338), OPEN).** The entry above says *"Landing this is also the stated trigger for revisiting the Dynamic Type finding."* The fonts landed; nobody revisited. Measured 2026-09-19:
  - **The fix works.** Replacing `Font(UIFontMetrics(forTextStyle:).scaledFont(for:))` with `Font.custom(_:size:relativeTo:)` clears `Dynamic Type font sizes are unsupported` — `ForcedUpdateAccessibilityUITests.testUnfilteredAuditRecordsOnlyTheAppWideDynamicTypeFinding` passes with its `XCTExpectFailure` removed, and the largest-Dynamic-Type audit keeps passing.
  - **And it breaks layout, for a reason specific to these fonts.** Both faces are **variable with a single `wght` axis, 400–800, default 400** (probed directly from the `.ttf`s). Weight is reachable only by setting that axis on a `UIFontDescriptor`, and a descriptor cannot travel through `Font.custom`. SwiftUI synthesises bold instead, glyph metrics change, and `testNoStoreLinkStatePassesTheAccessibilityAudit` — green before — fails with `Text clipped`.
  - **This is not a new hazard.** `OmenTypography.swift`'s own header records that the same class of mistake made *"every role render at 400 and the whole ramp go flat"* as **shipped behaviour on 2026-09-15**, and notes it is invisible to any test asserting a family name, because the family name is identical either way.
  - **What an actual fix needs**, so the next attempt does not rediscover this: the faces carry named instances (Regular/Medium/SemiBold/Bold/ExtraBold), but `UIAppFonts` registration exposes only the **default instance's** PostScript name. Making `Font.custom` viable means registering each named instance and routing roles to them — a real piece of work, not a one-line change. It needs a **rendered-weight assertion** (family-name assertions cannot see this failure) and a before/after visual pass across all thirty screens.
  - **Deliberately deferred 2026-09-19 (founder):** not undertaken while four screen journeys are being built against point-exact artboards under a D11 no-scroll constraint. The defect is **audit visibility, not user-facing breakage** — text does scale and reflow correctly at runtime — so the cost of waiting is bounded, while the cost of getting it wrong is the ramp going flat again, undetected.
