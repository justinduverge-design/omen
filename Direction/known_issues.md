...
- ~~**Every scored move is graded as PPR regardless of league settings (found 2026-08-15, tracked as `A6-MovesScoringFormat`).**~~ **FIXED ON `codex/a6-a7-football-data` 2026-08-24; not merged, migrated, or deployed.** New recommendations capture and persist canonical `Standard`, `Half PPR`, or `PPR` league scoring, and Tuesday grading consumes that field; only null/absent historical rows retain the PPR compatibility fallback. The review-only nullable additive SQL is `sql/2026-08-24_a6_moves_scoring_format_review.sql` and has not been applied. This issue is not operationally resolved until founder review, staging migration + verification, later production approval/application, and code deployment occur in that order.
