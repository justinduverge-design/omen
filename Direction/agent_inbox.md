...
**Refreshed 2026-07-04 (canonical off-season signal built locally; Phase 1.13 marked complete per Justin correction).** Phase 1.14 is complete locally (Codex): PR #75 and PR #76 deploy runs both completed successfully, current prod serves the transparent Omen lockup bundle/asset, and `.github/workflows/deploy.yml` now has a post-health bundle-level logo verification step. Handoff: `Blueprints/handoffs/2026-07-04-phase1-14-deploy-logo-verification.md`. Phase 1.13 is complete and merged via PR #75; remaining mobile/ARIA polish items are follow-ups, not Phase 1.13 blockers. The Cowork mobile-QA raw-JSON P1 is fixed locally; the Standings off-season P1 is now fixed locally by the shared backend signal, but neither local fix has been pushed, merged, or deployed yet.
...
1. **Phase 2.9 — Account delete UI.** Expose backend route at `src/routes/userPrivacy.js:136` in `Account.jsx`. Confirmation phrase: "DELETE MY OMEN DATA". Done docs: feature + page + design + security.
2. **Phase 2.10 — Trade share card.** Share button on Trade Analyzer result, server-side OG image. Unblocked (Backend Phase 2.10 hash routes deployed). Done docs: feature + page + design + recommendation.
3. **Phase 2.18 — Waiver Wire route activation.** Free-tier route currently hidden behind doctrine-stale `ProGate`; 6 focused sub-tasks in `current_sprint.md`. Unblocked (Phase 1.6 chip helper shipped). Done docs: feature + page + design + recommendation.
4. **Phase 1.15 — Post-deploy visual smoke on prod.** Add a `slops-canary`-driven post-deploy smoke against `slopssaloon.com` for home/about/login logo visibility and no `[C]` placeholder / black-rectangle regression. Cost: small. Done docs: release.
5. **Phase 2.11 — FP1 signal-honesty labels.** Surface each Omen input's `live` / `stub` / `unavailable` status. Backend vocabulary already exists at `src/services/omen.js:356`. Cost: medium. Done docs: feature + page + design + recommendation.
...
- **Both Cowork mobile-QA P1s are resolved locally, but still need Justin-gated commit/push/merge/deploy.** (a) Unhandled 429 raw-JSON leak — root cause was `src/server.js`'s general rate limiter mounted globally instead of scoped to `/api/*`; fixed and live-verified (150/150 static requests exempt, API limiter still functional at 100/min). Local, not committed — see `Blueprints/handoffs/2026-07-04-ratelimit-scope-and-standings-offseason-triage.md`. (b) Standings-fails-to-load — fixed locally by the canonical off-season signal on branch `codex/canonical-offseason-signal`; `GET /api/league/standings` now returns a successful empty standings envelope during off-season before provider calls. Contract: `Blueprints/handoffs/backend-to-frontend.md` ("Canonical Off-Season Signal — 2026-07-04").
