...
freshness_triggers:
  ...
  - deploy-immutable-tags.patch lands or changes
...
**Valid against Omen `origin/main` at `e8fb09e` on 2026-08-20.**
...
| Is the deploy fix on `main`? | No. The patch is preserved, approved, and prepared, but `.github/workflows/deploy.yml` still publishes only mutable `:main` tags and still runs an unscoped `docker image prune -f`. |
| What is the live backend rollback path today? | GHCR digest lookup and a temporary `:rollback` tag. It requires package-read access. |
| What becomes primary after the patch lands? | The immutable `sha-<full-commit-sha>` image tag. Digest lookup remains the fallback for older builds with no SHA tag. |
| Has the rollback procedure been operationally proven? | No. The founder-only non-critical live exercise has not run. |
| Can O2 close now? | No. The deploy change is not applied and the required live exercise has no evidence. |
...
O2 task                  IN_PROGRESS
  |
  +-- deploy fix         PREPARED_NOT_APPLIED
  |     |
  |     +-- land patch   APPLIED
  |
...
- The immutable-tag patch must land before the SHA-tag procedure becomes the live path.
- The exercise requires explicit founder execution and a non-critical production deploy.

### Enables

- Landing the patch enables a readable rollback target and preserves one week of local image history.
...
1. An authorized implementation session lands `deploy-immutable-tags.patch` and verifies the workflow result.
2. That session updates the runbook so SHA tags are primary and digest lookup is the legacy fallback.
3. The sprint keeps `O2` as `IN_PROGRESS`; only `change_state` advances to `APPLIED`.
4. Justin runs the documented six-step exercise against a non-critical deploy and records timings plus `/api/version` evidence for rollback and roll-forward.
5. Run `node scripts/check-sprint-staleness.js` and read its coverage block before any closure.
6. If all Done gates are satisfied, add the ledger and completed-sprint receipts and close O2 under the canonical status model.
...
2. **Metadata value:** useful. Separating task, implementation, and exercise states removes the handoff's largest ambiguity.
