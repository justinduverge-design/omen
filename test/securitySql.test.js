...
test("retired paid-access schema is removed idempotently", () => {
  const sql = readRepoFile("sql", "omen_rls_security.sql");
  const compactSql = sql.replace(/\s+/g, " ");

  assert.match(compactSql, /drop table if exists public\.subscriptions;/i);
  assert.match(compactSql, /alter table public\.users drop column if exists is_subscribed;/i);
  assert.doesNotMatch(compactSql, /create table if not exists public\.subscriptions/i);
});
