...
} = require("../src/services/yahooDraft");
...
test("buildYahooDraftId and parseYahooDraftId round-trip a league key", () => {
  const draftId = buildYahooDraftId("449.l.12345");
  assert.equal(draftId, "yahoo:449.l.12345");
  assert.equal(parseYahooDraftId(draftId), "449.l.12345");
  assert.equal(parseYahooDraftId("bad-value"), null);
});

test("buildDraftListResponse emits yahoo-draft-list.v1", () => {
  const res = buildDraftListResponse({
    leagueKey: "449.l.12345",
    draft: draftSnapshot(),
  });

  assert.equal(res.contract_version, YAHOO_DRAFT_LIST_VERSION);
  assert.equal(res.league_key, "449.l.12345");
  assert.equal(res.drafts.length, 1);
  assert.equal(res.drafts[0].draft_id, "yahoo:449.l.12345");
...
test("buildDraftMetaResponse preserves nullable slot metadata", () => {
  ...
  assert.equal(res.contract_version, YAHOO_DRAFT_META_VERSION);
  assert.equal(res.draft_id, "yahoo:449.l.12345");
  assert.equal(res.draft.user_draft_slot, null);
  assert.equal(res.draft.slot_to_roster_id, null);
  assert.equal(res.draft.settings.pick_timer, 60);
});
...
test("buildDraftStateResponse returns full state with cursor delta", () => {
  ...
  assert.equal(res.contract_version, YAHOO_DRAFT_STATE_VERSION);
...
test("buildDraftStateResponse uses a low-frequency hint when complete", () => {
  const res = buildDraftStateResponse({
    ...
    draft: draftSnapshot({
      status: "complete",
      settings: { teams: 1, rounds: 2, pick_timer: 60 },
      picks: [
        {
          pick_no: 1,
          round: 1,
