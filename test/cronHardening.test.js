...
const cron = require("../src/corvus_tuesday_cron");

const root = path.join(__dirname, "..");

test("Tuesday cron is disabled unless explicitly enabled", () => {
  assert.equal(cron.isScoringEnabled({}), false);
  assert.equal(cron.isScoringEnabled({ CORVUS_CRON_SCORING_ENABLED: "false" }), false);
...
test("cron Dockerfile runs the Corvus worker from the crond spool path", () => {
  const dockerfile = fs.readFileSync(path.join(root, "Dockerfile.cron"), "utf8");

  assert.match(dockerfile, /node \/app\/src\/corvus_tuesday_cron\.js/);
...
test("cron runtime config keeps scoring explicitly gated", () => {
  ...
  assert.match(compose, /CORVUS_CRON_SCORING_ENABLED:\s*\$\{CORVUS_CRON_SCORING_ENABLED:-false\}/);
  assert.match(compose, /SPORTRADAR_API_KEY:\s*\$\{SPORTRADAR_API_KEY\}/);
  assert.match(envExample, /CORVUS_CRON_SCORING_ENABLED=false/);
